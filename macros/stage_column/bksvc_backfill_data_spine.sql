{%- macro bksvc_backfill_date_spine(model_name, tsf_model_name) -%}
...
select 
    RAW_DATE.date_actual as_of_date,
    cte_contracts.contract_id contract_id,
    concat(cte_contracts.contract_id, '-',  replace(RAW_DATE.date_actual ,'-','')) contract_date_id,
    imported_at,
  ...
  from RAW.BKSVC.RAW_DATE
    join cte_contracts 
        on cte_contracts.max_imported_at_by_month = cte_contracts.imported_at
        and RAW_DATE.last_day_of_month = cte_contracts.as_of_date
    
    left join RAW.BKSVC.RAW_CLIENT client on cte_contracts.client_id = client.client_id
         
  where
    RAW_DATE.date_Actual between  min_as_of_date and dateadd(day, -1, current_date())
